...
export const useTeamActivityFeed = (config: DashboardConfig) => {
  const jonFeed = useActivityFeed(config, TEAM_MEMBERS[0].githubUsername);
  const michaelFeed = useActivityFeed(config, TEAM_MEMBERS[1].githubUsername);
  const steveFeed = useActivityFeed(config, TEAM_MEMBERS[2].githubUsername);

  const feeds = [jonFeed, michaelFeed, steveFeed];
  ...
  }, [jonFeed.activities, michaelFeed.activities, steveFeed.activities]);
